...
class FolderApp:
    ...
    def trigger_move(self, tree):
        ...
        if selected_item:
            ...
            source_path = current_data[1]
            tags = current_data[2]
            target_path = current_data[3]
    ...
    def refresh_watches(self, refresh_ids=[]):
        ...
        for tagpath in self.tags:
            ...
            if len(list(filter(lambda x: x.meta_id == id, self.watches))) == 0:
                watcher = FileWatcher(source=tagpath.sourcepath, tags=tagpath.tags, target=tagpath.targetpath, meta_id=id, callback=self.watcher_callback)
