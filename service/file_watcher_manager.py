...
class FileWatcherManager(object):
    ...
    def upsert_observer(self, source_id, source, ignore_directories, operations, conditions, auto_start=True):
        ...
        data_to_hash = f"{source}{ignore_directories}{operations_str}{conditions_str}"
        source_hash = hashlib.sha256(data_to_hash.encode()).hexdigest()
        matched_observer = next((observer for observer in self.__file_observers if observer.source_hash == source_id), None)
        add = True
        if matched_observer:
            if matched_observer.source_hash != source_hash:
                ...
                add = False
        if add:
            file_observer = FileObserver(source_id, source, ignore_directories, operations, conditions)
